...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div class="mb-8">
        <div class="flex items-center space-x-4 mb-4">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Create Post Category</h1>
        </div>
        <p class="text-gray-600">Tạo danh mục mới cho bài viết</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.post-categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#005366] focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="flex space-x-2">
                        <input type="color" id="color" name="color" value="{{ old('color', '#005366') }}"
                               class="w-16 h-12 border border-gray-300 rounded-lg cursor-pointer">
                        <input type="text" id="color-text" name="color" value="{{ old('color', '#005366') }}"
            ...
            <div class="border-t pt-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="meta_description" name="meta_description" rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#005366] focus:border-transparent">{{ old('meta_description') }}</textarea>
                </div>
            </div>
        </div>
