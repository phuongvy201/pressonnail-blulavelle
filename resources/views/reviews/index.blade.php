...
<main class="min-h-screen bg-[#f8f6f6]" style="font-family:'Plus Jakarta Sans',system-ui,sans-serif;">
    <div class="max-w-[920px] mx-auto w-full px-4 sm:px-6 py-8 md:py-12 pb-20">
        <nav class="text-sm text-slate-500 mb-6 flex flex-wrap items-center gap-2">
            ...
            <span>/</span>
            <span class="text-slate-800 font-medium">Customer reviews</span>
        </nav>
        ...
        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
            <div class="p-6 md:p-10">
                <h1 class="text-xl md:text-2xl font-extrabold text-slate-900 tracking-tight">
                    All customer reviews ({{ $countLabel }})
                </h1>

                @if($totalReviews === 0)
                    <div class="mt-10 text-center py-16 px-4">
                        <span class="material-symbols-outlined text-6xl text-slate-200">rate_review</span>
                        <p class="mt-4 text-slate-600 font-medium">There are no published reviews yet.</p>
                        <a href="{{ route('home') }}" class="inline-flex mt-6 items-center gap-2 px-6 py-3 rounded-xl bg-[#0297FE] text-white font-bold hover:opacity-90 transition-opacity">
                            Back to home
                        </a>
                    </div>
                @else
                    <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                        <div>
                            ...
                            <p class="mt-4 text-sm text-slate-500 leading-relaxed">
                                The average rating is calculated from all approved reviews across the store.
                            </p>
                        </div>
                    ...
                    <form method="get" action="{{ route('reviews.public') }}" id="platform-reviews-filters" class="mt-8 flex flex-wrap items-center gap-3">
                        <div class="relative">
                    ...
                    <div class="mt-10 divide-y divide-slate-100">
                        ...
                        <article class="py-8 first:pt-0">
                            ...
                            <p class="text-xs text-slate-500 mb-2">
                                ...
                                <a href="{{ route('shops.show', $review->product->shop->shop_slug) }}" class="font-semibold text-slate-700 hover:text-[#0297FE] hover:underline">{{ $review->product->shop->shop_name }}</a>
                            </p>
                            @endif
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-100 text-center">
                        <a href="{{ route('home') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-[#0297FE] hover:underline">
