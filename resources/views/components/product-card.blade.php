        ...
        @if($showQuickView)
            <a href="{{ route('products.show', $product->slug) }}"
               class="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <span class="inline-flex items-center gap-2 px-5 py-3 bg-[#f0427c] text-white font-semibold rounded-full shadow-xl transform translate-y-3 group-hover:translate-y-0 transition-transform duration-300 hover:bg-[#e03a6f]">
    ...
    <div class="p-3 sm:p-4">
        ...
        <div class="flex flex-wrap items-baseline gap-2">
            @if($onSale)
                ...
                <span class="text-xs sm:text-sm text-gray-400 line-through">{{ format_price_usd($originalPriceUsd) }}</span>
            @else
