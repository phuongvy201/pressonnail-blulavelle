...
<div class="px-8 pt-10 pb-6 flex flex-col items-center text-center">
    ...
    <p class="text-slate-500 mt-2">Log in to your account to manage your orders</p>
</div>
...
{{-- Login Form --}}
<form method="POST" action="{{ route('login') }}" class="px-8 pb-10 space-y-5">
    @csrf
    ...
    <div>
        <div class="flex justify-between mb-1.5">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm font-semibold text-primary hover:text-primary/80">Forgot password?</a>
            @endif
        </div>
        <div class="relative">
            ...
            <button type="button" onclick="togglePassword()" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600" aria-label="Toggle password visibility">
    ...
    <p class="text-center text-sm text-slate-600 mt-6">
        ...
        <a href="{{ route('register') }}" class="font-bold text-primary hover:text-primary/80">Create an account</a>
    </p>
</form>
